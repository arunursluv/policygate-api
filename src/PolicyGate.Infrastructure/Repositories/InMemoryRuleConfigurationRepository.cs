...
public sealed class InMemoryRuleConfigurationRepository : IRuleConfigurationRepository
{
    ...
    public InMemoryRuleConfigurationRepository(IEnumerable<SecurityRuleDefinition> initial) { foreach (var item in initial) definitions[item.RuleId] = item; }
    ...
    public bool Update(string ruleId, bool enabled) { if (!definitions.TryGetValue(ruleId, out var current)) return false; definitions[ruleId] = current with { Enabled = enabled }; return true; }
}
