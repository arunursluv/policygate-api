...
public sealed class InMemoryAuditRepository : IAuditRepository
{
    ...
    {
        cancellationToken.ThrowIfCancellationRequested(); events.Enqueue(auditEvent);
        return Task.CompletedTask;
    }
    public Task<IReadOnlyCollection<AuditEvent>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult<IReadOnlyCollection<AuditEvent>>(events.OrderByDescending(x => x.TimestampUtc).ToArray());
    }
